...
def show_analysis():
    ...
    try:
        ...
        analysis_mode = st.radio(
            "Analysis Mode",
            ["Portfolio Analysis", "Wallet Analysis"],
            horizontal=True
        )
...
def show_all_wallets_auto_update(df: pd.DataFrame):
    ...
    if start_auto:
        ...
        try:
            while (st.session_state.get("wallet_auto_update_running", False) and
                   st.session_state.get("wallet_update_count", 0) < max_updates):
...
def show_single_wallet_auto_update(df: pd.DataFrame, available_wallets: list):
    ...
    if start_auto:
        ...
        try:
            while (st.session_state.get("single_wallet_auto_running", False) and
                   st.session_state.get("single_wallet_update_count", 0) < max_updates):
